...
export async function subscribeToChannelWebhooks(channelId: string) {
  ...
  console.log(`[WebhookSub] Requesting subscription for ${channelId} to ${callbackUrl}...`);
  ...
  try {
    ...
    if (!response.ok) {
      ...
      console.error(`[WebhookSub] Google Hub error for ${channelId}:`, errorText);
      return { success: false, error: errorText };
    }

    console.log(`[WebhookSub] Subscription requested successfully for ${channelId}.`);
    return { success: true };
  } catch (error) {
