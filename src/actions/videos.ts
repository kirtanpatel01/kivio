...
export function mapDbVideoToYouTubeVideo(v: any, channelInfo?: { title: string; thumbnailMedium: string }): YouTubeVideo | null {
    if (!v) return null;

    // 1. Ensure we have objects, not strings (defensive against driver quirks)
    const rawVideo = typeof v.rawVideo === "string" ? JSON.parse(v.rawVideo) : v.rawVideo;
    const rawPlaylistItem = typeof v.rawPlaylistItem === "string" ? JSON.parse(v.rawPlaylistItem) : v.rawPlaylistItem;
    ...
    return {
        ...
        duration: rawContentDetails?.duration ? parseISO8601Duration(rawContentDetails.duration) : null,
        ...
        channelAvatar: channelInfo?.thumbnailMedium ?? v.youtubeChannel?.thumbnailMedium ?? "",
