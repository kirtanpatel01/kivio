 ...
 */
export async function syncVideoAndNotifySubscribers(videoId: string, channelId: string) {
    const apiKey = getEnvVar("YOUTUBE_API_KEY")?.trim();
    if (!apiKey) throw new Error("YouTube API Key not configured.");

    console.log(`[WebhookSync] Processing video ${videoId} for channel ${channelId}...`);
    ...
    try {
        ...
        if (!detailsRes.ok) throw new Error(`YouTube API error: ${await detailsRes.text()}`);
        
        const detailsData = await detailsRes.json();
        const item = detailsData.items?.[0];
        if (!item) {
            console.warn(`[WebhookSync] Video ${videoId} not found in API. It might be private or deleted.`);
            return;
        }

        // 2. Prepare Video Metadata
        const durationIso: string | undefined = item?.contentDetails?.duration;
        const durationSeconds = durationIso ? parseISO8601DurationToSeconds(durationIso) : null;
        ...
        if (!yc) {
            console.warn(`[WebhookSync] Channel ${channelId} not found in our database. Skipping notifications.`);
            return;
        }
        ...
        console.log(`[WebhookSync] Successfully synced video and created ${notificationRows.length} notifications.`);
    } catch (error) {
